...
def build_anchor_text(
    ...
    for axis in TEXT_AXIS_PRIORITY:
        ...
        value = float(selected_profile[axis])
        b = bucket(value)

        if b == "mid":
            continue

        strength = abs(value - 50)
        text = get_fragment(fragments, "anchor", axis, b)

        if text:
            candidates.append((strength, text))

    top = [text for _, text in sorted(candidates, reverse=True)[:3]]

    if not top:
        return "Your selected artists point toward a balanced taste profile."

    return f"Your selected artists point toward {join_naturally(top)}."


def build_match_text(
    fragments: dict[tuple[str, str, str], str],
    selected_profile: pd.Series,
    recommended_profile: pd.Series,
    feature_cols: list[str],
) -> str:
    shared = []
    contrasts = []
    ...
    for axis in TEXT_AXIS_PRIORITY:
        ...
        selected_value = float(selected_profile[axis])
        recommended_value = float(recommended_profile[axis])
    ...
    contrast_texts = [text for _, text in sorted(contrasts, reverse=True)[:2]]

    parts = []

    if shared_texts:
        parts.append(f"It {join_naturally(shared_texts)}")

    if contrast_texts:
        parts.append(f"while it {join_naturally(contrast_texts)}")

    if not parts:
        return "It sits very close to your selected taste profile."
...
def build_reason(
    ...
    anchor = build_anchor_text(
        fragments=fragments,
        selected_profile=cluster_center,
        feature_cols=feature_cols,
    )

    match = build_match_text(
        fragments=fragments,
        selected_profile=cluster_center,
        recommended_profile=recommendation_row,
        feature_cols=feature_cols,
    )

    return (
        f"{anchor} {match} "
        f"Closest reference in your input: {support_artist}. Genre: {genre}."
    )
